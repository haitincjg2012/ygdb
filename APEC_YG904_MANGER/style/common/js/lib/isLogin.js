...
define(['jquery', 'global', 'dialog'], function ($, global) {
	...
	mod.login = function () {
		if (isLoad != false) {
			...
			var cssUrl2 = global.static_url + "common/css/loginframe.css";
		...
		loginhtml += '<li class="m-tip"><span class="J-tip">请输入密码! </span></li>';
					  ...
					  function (data) {
						  if (data.success === true) {
							  ...
							  if (document.getElementById('collect')) {
								  var goodsCommonId = $("#collect").data('id');
								  $.post('/home/checkGoodsCollect', {goodsCommonId: goodsCommonId}, function (data) {
									  if (data.result == 'SUCCESS') {
										  $("#collect").addClass('z-collected');
									  }
								  }, 'json');
							  }
