...
//用户的发布的供求列表
router.post('/_node_product/_user_off_list' + config.urlSuffix, bodyParser, function(req, res, next) {
   ...
   return ef(done, bind$(redis, 'get'), config.userTokenPrefix + token, function(json){
       ...
       return ef(done, bind$(redis, 'hget'), config.userInfoPrefix + json, config.userProductOffKey , function(obj){
           var arr = [];
           ...
           //Search 参数拼装
           var searchParams = {
              index: config.productOffIndex.index,
              type: config.productOffIndex.type,
              body: {
                  ids: pageArr
              }
          };
