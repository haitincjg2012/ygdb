...
var listData = require('./listdata-agency');

//找代办的列表查询
router.post('/_node_user_org/_agency_list' + config.urlSuffix, bodyParser, function(req, res, next) {
   ...
   //Search 参数拼装
   var searchParams = {
      index: config.orgAgencyIndex.index,
      type: config.orgAgencyIndex.type,
      body: {
        query:{
          bool:{}
        },
  ...
  if(flag){
    ...
    searchParams.body.query.bool.must =  {
         multi_match: {
           ...
           fields: config.orgAgencySearchFields,
           type:  "cross_fields",
           operator: "and" ,
  ...
  console.log("#############API:/_node_user_org/_agency_list/ [Node query]: ")
