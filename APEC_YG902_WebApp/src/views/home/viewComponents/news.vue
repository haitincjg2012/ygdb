...
<template>
  <div class="newsContainer">
    ...
    <my-scroll class="newsScroll" :pullup="pullup" @scrollToEnd="loadMore">
      <div class="newsContent">
        <!--轮播-->
        <div class="carousel">
          <mt-swipe :auto="4000">
            <mt-swipe-item v-for="item in carouselFilter" :key="item.id">
                <img :src="item.imgsrc"/><!--
        ...
        <!--列表-->
        <!--<ul class="newsList" v-for="item in newsData|listFilter">-->
        <ul class="newsList">
          <li v-for="item in listFilter" :key="item.id" @click="goDetail(item.id)">
            <div class="newsItem">
              ...
              <div class="newslistL">
                <h2>{{item.title}}</h2>
                <p>{{item.websit}}<span>{{item.peroid}}</span></p>
              </div>
            </div>
          </li>
        </ul>
        ...
        data(){
            return {
              ...
              newsData:[//新闻列表
                ...
                {id:12,title:"2017年红富士苹果走货令人担忧",imgsrc:imgsrc3,websit:"国内网",peroid:"12小时前"}
              ],
              /*分页参数--上拉加载更多*/
              pullup:true,
              loadMflag:true,
