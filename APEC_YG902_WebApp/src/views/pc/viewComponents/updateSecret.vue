        ...
        <div class="loggin-panel">
          <form onsubmit="return false">
            <div class="phone-input">
              <span>原始密码</span>
              <input id="input-code-orginal" oninvalid="setCustomValidity(' ')" class="pure-input-1" type="password" v-model="orCode" placeholder="请输入登录密码"
                     oninput="setCustomValidity('')" required  minlength="6"/>
              <img @click.stop="openOrText($event,'input-code-orginal')" class="i-password-hide" :src="imgOrSrc">
            </div>
            <div class="dash-line">
            </div>
            ...
            <div class="login-btn">
              <input :class="loginBtnCls" type="submit" id="btn-login-code" value="确认修改" @click="update"></input>
            </div>
          </form>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      openOrText(event,dom){
        var demoInput = document.getElementById(dom);
        //隐藏text block，显示password block
        if (demoInput.type == "password") {
          demoInput.type = "text";
          this.imgOrSrc = showImgSrc;
        }else {
          demoInput.type = "password";
          this.imgOrSrc = hideImgSrc;
        }
      },
      openText(event,dom){
        var demoInput = document.getElementById(dom);
      ...
      update(){
        ...
        let params = {
          api: "/yg-user-service/user/updatePassword.apec",
          data: {
            "oldPassword":self.orCode,
            "password":self.code,
